...
<nav x-data="{ open: false }" class="bg-[#0B1220] border-b border-blue-500/20 font-oxanium relative">
            ...
            <div class="flex items-center gap-8">
                ...
                @if (Auth::check() && Auth::user()->role === 'admin')
                    <div class="hidden sm:flex items-center gap-2">
                        ...
                        <!-- EQUIPOS -->
                        <x-nav-link :href="route('equipos.listar')" :active="request()->routeIs('equipos.*')"
                        ...
                        <!-- ENTRENADORES -->
                        <x-nav-link :href="route('entrenadores.listar')" :active="request()->routeIs('entrenadores.*')"
                            class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                ...
                @if (Auth::check() && Auth::user()->role === 'entrenador')
                    <div class="hidden sm:flex items-center gap-2">
                        ...
                        @if ($equipo)
                            ...
                            <!-- EQUIPO -->
                            <x-nav-link :href="route('equipos.ver', $equipo)" :active="request()->routeIs('equipos.ver')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                                <span class="{{ request()->routeIs('equipos.ver') ? 'text-white' : 'text-white/40 group-hover/link:text-white' }} transition-colors duration-300">
                                    {{ __('Equipo') }}
                                </span>
                                <div class="absolute -bottom-1 left-0 h-[2px] bg-blue-600 shadow-[0_0_20px_#2563eb] transition-all duration-500"
                                    style="{{ request()->routeIs('equipos.ver') ? 'width: 100%; opacity: 1;' : 'width: 0%; opacity: 0;' }}">
                                </div>
                            </x-nav-link>

                            <!-- ENTRENO -->
                            <x-nav-link :href="route('entrenamientos.listar', $equipo)" :active="request()->routeIs('entrenamientos.*')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                            ...
                            <!-- PARTIDO -->
                            <x-nav-link :href="route('partidos.listar', $equipo)" :active="request()->routeIs('partidos.*')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                            ...
                            <!-- INSCRIPCIÓN -->
                            <x-nav-link :href="route('pagos.listar', $equipo)" :active="request()->routeIs('pagos.*')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                ...
                @if (Auth::check() && Auth::user()->role === 'jugador')
                    <div class="hidden sm:flex items-center gap-2">
                        ...
                        @if (!request()->routeIs('equipos.ver', 'entrenamientos.*', 'partidos.*', 'pagos.show'))
                            <!-- INICIO -->
                            <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                        ...
                        @if ($equipo)
                            ...
                            @endphp
                            @if ($jugador)
                            <x-nav-link :href="route('pagos.ver', [$equipo, $jugador])" :active="request()->routeIs('pagos.ver')"
                                class="relative group/link px-5 py-2 text-xs font-black uppercase tracking-[0.4em] transition-all border-none">
                                <span class="{{ request()->routeIs('pagos.ver') ? 'text-white' : 'text-white/40 group-hover/link:text-white' }} transition-colors duration-300">
                                    {{ __('Inscripción') }}
                                </span>
                                <div class="absolute -bottom-1 left-0 h-[2px] bg-blue-600 shadow-[0_0_20px_#2563eb] transition-all duration-500"
                                    style="{{ request()->routeIs('pagos.ver') ? 'width: 100%; opacity: 1;' : 'width: 0%; opacity: 0;' }}">
                                </div>
                            </x-nav-link>
                            @endif
                        @else
                        ...
                        <div class="relative z-50 mt-2 w-48 bg-[#0B1220]/95 backdrop-blur-xl border border-blue-500/40 shadow-[0_0_20px_rgba(37,99,235,0.2)]">
                            ...
                            <div class="p-1 space-y-0.5 font-oxanium">
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="inline">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full text-left px-3 py-2 text-xs uppercase tracking-[0.2em] text-red-400/70 hover:text-red-400 hover:bg-red-500/10 transition-all border-l-2 border-transparent hover:border-red-500 cursor-pointer">
                                        Salir_X
                                    </button>
                                </form>
                            </div>
    ...
    <div x-show="open" x-transition class="sm:hidden bg-[#0B1220]/95 backdrop-blur-xl border-t border-blue-500/10">
        <div class="pt-4 pb-4 space-y-1 px-4">
            ...
            @if(Auth::check() && Auth::user()->role === 'admin')
                <x-responsive-nav-link :href="route('equipos.listar')" :active="request()->routeIs('equipos.*')"
                    class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                    {{ __('// Equipos') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('entrenadores.listar')" :active="request()->routeIs('entrenadores.*')"
                    class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                    {{ __('// Entrenadores') }}
                </x-responsive-nav-link>
            @endif
            @if(Auth::check() && Auth::user()->role === 'jugador')
                @php $equipo = Auth::user()->primerEquipoAceptado(); @endphp
                @if($equipo)
                    <x-responsive-nav-link :href="route('equipos.ver', $equipo)" :active="request()->routeIs('equipos.ver')"
                        class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                        {{ __('// Equipo') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('entrenamientos.listar', $equipo)" :active="request()->routeIs('entrenamientos.*')"
                        class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                        {{ __('// Entreno') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('partidos.listar', $equipo)" :active="request()->routeIs('partidos.*')"
                        class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                        {{ __('// Partido') }}
                    </x-responsive-nav-link>
                    @php
                    ...
                    @endphp
                    @if($jugador)
                    <x-responsive-nav-link :href="route('pagos.ver', [$equipo, $jugador])" :active="request()->routeIs('pagos.ver')"
                        class="block text-xs font-bold uppercase tracking-[0.4em] text-white border-none py-3">
                        {{ __('// Inscripción') }}
                    </x-responsive-nav-link>
                    @endif
            ...
            <!-- SEPARADOR PERFIL MÓVIL -->
            <div class="border-t border-blue-500/10 my-3"></div>

            <!-- PERFIL MÓVIL -->
            <div class="space-y-1 px-1">
                <div class="px-3 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white/70">
                    {{ Auth::user()->name }}
                </div>
                <x-responsive-nav-link :href="route('profile.edit')"
                    class="block text-xs uppercase tracking-[0.2em] text-white/70 hover:text-white py-2 px-3 hover:bg-blue-600/30 transition-all">
                    Mi_ID
                </x-responsive-nav-link>
                <form method="POST" action="{{ route('logout') }}" class="block">
                    @csrf
                    <button type="submit"
                        class="w-full text-left text-xs uppercase tracking-[0.2em] text-red-400/70 hover:text-red-400 py-2 px-3 hover:bg-red-500/10 transition-all cursor-pointer">
                        Salir_X
                    </button>
                </form>
            </div>
